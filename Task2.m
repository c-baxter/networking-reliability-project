...
for attempts = 1:numColumns(2)
    for probability = 1:99
        simulations(probability, attempts) = runTwoSeriesLinkSim(numPackets(attempts), probability/100, numIterations);
        calculations(probability, attempts) = numPackets(attempts)*(2-(probability/100))/((1-(probability/100))^2);
    end
end
